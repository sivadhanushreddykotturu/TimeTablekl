...
export default function Attendance() {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        // Handle session refresh — update stored cookies if backend auto-healed
  ...
  // The new backend returns snake_case; old data may have ERP PascalCase.
  // This resolves either format transparently.
                      ...
                      {["L", "P", "S", "T"].includes(section.ltps.charAt(0).toUpperCase()) && (
                        <div
                          ...
                          onClick={async () => {
                            ...
                            // Lazy-fetch register details via register_href (stored URL)
                            if (attendanceItem.register_href) {
                              ...
                              try {
                                ...
                                // Handle session refresh — persist fresh cookies if auto-healed
                                if (!friendCredentials) {
                                  handleSessionRefresh(res.data);
                                }
                              ...
                              // Fallback: no register_href available
                              handleRegisterClick({
                                metadata: {
                                  Coursecode: attendanceItem.course_code ?? attendanceItem.Coursecode,
